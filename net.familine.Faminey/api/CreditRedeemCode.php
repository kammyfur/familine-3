...
$profile->credit = $profile->credit + $sel->value;
array_unshift($profile->history, [
    "name" => "Code cadeau de " . number_format((float)$sel->value, 2, ',', '') . " €",
    "to" => $_USER,
    "cost" => $sel->value,
    "date" => date("d/m/Y"),
    "type" => "code",
...
file_put_contents($_SERVER["DOCUMENT_ROOT"] . "/net.familine.Faminey/private/codes.json", json_encode($codes, JSON_PRETTY_PRINT));
die("ok");